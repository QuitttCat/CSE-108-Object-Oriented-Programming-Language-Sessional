...
public:
    ...
    ~Circle();
    Circle operator*(int h)
    {
        Circle temp;
        temp.setCenter(this->center);
        temp.setRadius(h);
        return temp;
    }
};
...
public:
    ...
    friend RightCircularCone operator*(Circle c,int h);
    friend RightCircularCone operator*(Circle c,RightCircularCone rc);
};

// your code

 RightCircularCone operator*(Circle c,int h)
    {
        RightCircularCone temp;
        temp.setBase(c);
        temp.setVolume((1/3)*pi*c.getRadius()*c.getRadius()*h);
        return temp;
    }
RightCircularCone operator*(Circle c,RightCircularCone rc)
{
    RightCircularCone temp;
    temp.setBase(c);
    Circle tmp=rc.getBase();
    temp.setVolume(rc.getVolume()*c.getRadius()*c.getRadius()/(tmp.getRadius()*tmp.getRadius()));
    return temp;
}
