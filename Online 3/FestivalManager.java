...
public class FestivalManager {
    ...
    public void deleteEvent(String eventName)
    {
        ...
        {
            if(events[i].getEventName().equals(eventName)) deleteIndex=i;
        }
        if(deleteIndex==-1)
        {
            System.out.println("No event named "+eventName+" exists. Delete unsuccessful.");
        }
        else
        {
            for(int i=deleteIndex;i<eventsAdded-1;i++)
            {
                events[i]=events[i+1];
            }
            eventsAdded--;
    ...
    public void showInOrderOfMaxCapacity()
    {
        for(int i=0;i<eventsAdded-1;i++)
        {
            for(int j=0;i<eventsAdded-1-i;j++)
            {
                if(events[j].getMaximumParticipants()>=events[j+1].getMaximumParticipants())
                {
                    Event[] temp=new Event[1];
                    temp[0]=events[j];
                    events[j]=events[j+1];
                    events[j+1]=temp[0];
                }
